function representa_stateArray(stateArrayDisc, stateArrayCont, stateArrayOpt, gtpose, ransac_x, ransac_y)
% Representa en un plot un esquema de como sería la información que ve el
% robot
    num_figure = 3;
    dist_max_representada = stateArrayOpt.max_dist * 1.1;
    ...
    % Pinto el RANSAC
    ransac_x_mov = (ransac_x.*cos(gt_ang) - ransac_y.*sin(gt_ang) ) + gt_x;
    ransac_y_mov = (ransac_x.*sin(gt_ang) + ransac_y.*cos(gt_ang) ) + gt_y;

    plot(ransac_x_mov, ransac_y_mov, 'g-')
